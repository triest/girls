...
@section('content')
    ...
    <form action="{{route('girlsEdit')}}" method="post" enctype="multipart/form-data" novalidate>
        {{ csrf_field() }}
       ...
       Обновить фотографию:
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="name" name="name" placeholder="Имя" value="{{$girl->name}}" readonly required>
        </div>
        @if($errors->has('name'))
       ...
       @if($girl->sex="famele")
        ...
        <label for="age">Возраст:
            <input type="number" name="age" min="18" value="{{$girl->age}}" onkeypress="return isNumber2(event)" checked>
        </label><br>
        @if($errors->has('age'))
        ...
        <label for="age">Рост:
            <input type="number" name="height" min="100" value="{{$girl->height}}" onkeypress="return isNumber2(event)" >
        </label><br>
        <label for="age">Вес:
            <input type="number" name="weight" min="45" step="1" value="{{$girl->weight}}" pattern="[^@]+@[^@]+\.[0-9]{2,3}" onkeypress="return isNumber2(event)">
        </label><br>
